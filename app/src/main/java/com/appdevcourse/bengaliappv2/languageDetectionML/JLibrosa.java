package com.appdevcourse.bengaliappv2.languageDetectionML;
...
import com.appdevcourse.bengaliappv2.languageDetectionML.wavFile.WavFile;
import com.appdevcourse.bengaliappv2.languageDetectionML.wavFile.WavFileException;
import com.appdevcourse.bengaliappv2.languageDetectionML.exception.FileFormatNotSupportedException;
import com.appdevcourse.bengaliappv2.languageDetectionML.process.AudioFeatureExtraction;
...
public class JLibrosa {
	...
	public float[][] powerToDb(float[][] yValues, float ref, float topDb) {
		float[][] db = new float[yValues.length][yValues[0].length];
		float maxValue = Float.NEGATIVE_INFINITY;
		for (int i = 0; i < yValues.length; i++) {
			for (int j = 0; j < yValues[i].length; j++) {
				db[i][j] = (float) (10 * Math.log10(yValues[i][j] / ref));
				maxValue = Math.max(maxValue, db[i][j]);
			}
		}
		for (int i = 0; i < db.length; i++) {
			for (int j = 0; j < db[i].length; j++) {
				db[i][j] = Math.max(db[i][j], maxValue - topDb);
			}
		}
		return db;
